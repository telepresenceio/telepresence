...
// admin, itemizing the missing privileges in the report.
func (s *setupSuite) Test_SetupNonAdminHandoff() {
	...
	// Pins Attach=false via VPN-only mode, sidestepping the unrelated hard
	// webhook-denied error so only the softened privilege check is under
	// test; --attach is not a flag any more, so this goes through --input.
	inputFile := filepath.Join(dir, "input.yaml")
	rq.NoError(os.WriteFile(inputFile,
		[]byte("agentInjector:\n  enabled: false\nnodeAgent:\n  enabled: false\n"), 0o644))
	...
	s.Contains(stdout, "missing privileges listed above")
...
// Test_SetupClientRbacInputRoundTrip covers client RBAC as a pure --input
// concern: the engine has no opinion about clientRbac.* at all, so an input
// file's clientRbac.create/subjects carry straight into the --output file
// unchanged.
func (s *setupSuite) Test_SetupClientRbacInputRoundTrip() {
	ctx := s.Context()
	rq := s.Require()
	dir := itest.TempDir(ctx)
	inputFile := filepath.Join(dir, "input.yaml")
	...
	rq.NoError(os.WriteFile(inputFile, []byte(""+
		"clientRbac:\n"+
		"  create: true\n"+
		"  subjects:\n"+
		"    - kind: User\n"+
