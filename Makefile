...
go-test-teleproxy: go-get test-cluster
	$(FLOCK) firewall.lock go test -v -exec "sudo env PATH=${PATH} KUBECONFIG=$(abspath ${KUBECONFIG})" $(go.module)/cmd/teleproxy
go-test-other: go-get
	KUBECONFIG=$(abspath ${KUBECONFIG}) go test -v $(filter-out $(go.module)/internal/pkg/nat $(go.module)/cmd/teleproxy,$(go.pkgs))
