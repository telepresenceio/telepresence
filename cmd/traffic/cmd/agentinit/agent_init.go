...
// trafficAgentOwner returns the iptables owner-match flag and ID that single out traffic
// originating from the traffic-agent. The agent is identified by its primary group when
// AGENT_GID is set. The agent's UID may be shared with an application container (it is
// inherited from the app's securityContext unless explicitly configured), so when AGENT_GID
// is unset (the agent config was generated by an older traffic-manager), the UID-based
// identification is used as a fallback.
func trafficAgentOwner() (flag, id string, err error) {
	if gid, ok := os.LookupEnv(agentconfig.EnvAgentGID); ok && gid != "" {
		parsed, err := strconv.ParseUint(gid, 10, 32)
		if err != nil {
			return "", "", fmt.Errorf("invalid %s %q: %w", agentconfig.EnvAgentGID, gid, err)
		}
		return "--gid-owner", strconv.FormatUint(parsed, 10), nil
	}
	id, err = trafficAgentUID()
	if err != nil {
		return "", "", err
	}
	return "--uid-owner", id, nil
}
...
func (c *config) configureIptables(ctx context.Context, ipt iptablesConfigurer, loopback string, localHostCIDR netip.Prefix, podIP netip.Addr) error {
	...
	// The traffic-agent's own traffic is told apart from the application's by an iptables
	// owner match on the agent's primary group (or its UID, for configs generated by an
	// older traffic-manager).
	ownerFlag, ownerID, err := trafficAgentOwner()
	if err != nil {
		return err
	}
	...
	nextCn:
		...
		err = ipt.Insert(nat, "OUTPUT", 1,
			"-o", loopback,
			"-p", lcProto,
			"-m", "owner", "!", ownerFlag, ownerID,
			"-j", outputChain)
		if err != nil {
			return fmt.Errorf("failed to insert ! %s rule in OUTPUT: %w", ownerFlag, err)
		}
		outputInsertCount++
		...
		err = ipt.Insert(nat, "OUTPUT", 1,
			...
			"-j", outputChain)
		if err != nil {
			return fmt.Errorf("failed to insert pod IP ! %s rule in OUTPUT: %w", ownerFlag, err)
		}
		outputInsertCount++
	...
	return c.insertMeshBypassRules(ipt, ownerFlag, ownerID, 1+outputInsertCount, podIP.Is4())
}
...
func (c *config) insertMeshBypassRules(ipt iptablesConfigurer, ownerFlag, ownerID string, pos int, v4 bool) error {
...
// meshBypassTarget returns the target for the agent-owner mesh-bypass rules. When mesh-dial
