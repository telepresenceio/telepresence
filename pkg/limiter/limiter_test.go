...
func TestIntervalLimiter(fool *testing.T) {
	...
	t.expect(500*time.Millisecond, l.Limit(start.Add(500*time.Millisecond)))
	t.expect(1*time.Millisecond, l.Limit(start.Add(999*time.Millisecond)))
	t.expect(0, l.Limit(start.Add(1000*time.Millisecond)))
	t.expect(999*time.Millisecond, l.Limit(start.Add(1001*time.Millisecond)))
	t.expect(500*time.Millisecond, l.Limit(start.Add(1500*time.Millisecond)))
	t.expect(1*time.Millisecond, l.Limit(start.Add(1999*time.Millisecond)))
