...
func NewDialer(connID ConnID, bidiStream Tunnel, release func()) Handler {
	ttl := tcpConnTTL
...
func HandlerFromConn(connID ConnID, bidiStream Tunnel, release func(), conn net.Conn) Handler {
	ttl := tcpConnTTL
	if connID.Protocol() == ipproto.UDP {
		ttl = udpConnTTL
	}
...
func (h *dialer) sendTCD(ctx context.Context, code ControlCode) {
	...
	err := h.bidiStream.Send(ctx, ctrl)
	if err != nil {
...
func (h *dialer) readLoop(ctx context.Context) {
	...
	for {
		...
		if n > 0 {
			...
			err = h.bidiStream.Send(ctx, NewMessage(h.id, b[:n]))
			if err != nil {
				if ctx.Err() == nil {
