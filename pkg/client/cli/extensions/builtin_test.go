...
func TestTCPCompatible(t *testing.T) {
	...
	testcases := map[string]TestCase{
		...
		"invalid-name2":      {"", []string{"--tcp-bogus"}, nil, exactErr("unknown flag: --tcp-bogus")},
		"nil":                {"", nil, nil, assert.NoError},
		"empty":              {"", []string{}, nil, assert.NoError},
	}
	...
	for tcName, tc := range testcases {
		...
		t.Run(tcName, func(t *testing.T) {
			...
			outArgs, outErr := extensions.MakeArgsCompatible(ctx, "tcp", tc.InputImage, tc.InputArgs)
