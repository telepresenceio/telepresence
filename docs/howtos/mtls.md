...
Telepresence supports HTTP-filtered intercepts on both clear-text and TLS encrypted ports that use either HTTP/1.x and HTTP/2. Telepresence will automatically detect the protocol used by the application and use the appropriate protocol for the intercept. This is done by inspecting the Kubernetes service for the workload.

Telepresence will never use TLS or HTTP/2 for services that have a `spec.ports[].appProtocol` that is `tcp` or `udp`. Those values imply a "no app-layer sniffing" mode, so when set, they effectively rule out all uses of HTTP-filtered intercepts on encrypted traffic.

### TLS Detection
Telepresence will always use TLS for services that have:
...
- `spec.ports[<port>].appProtocol` that is `kubernetes.io/wss`, `wss`, `http2`, `https`, or `grpc`.
- `spec.ports[<port>].name` that is `https`.
- `spec.ports[<port>].port` that is 443.

Telepresence will never use TLS for services that have a `spec.ports[].appProtocol` that is `kubernetes.io/ws`, `kubernetes.io/h2c`, or `h2c`.
...
Telepresence will always use HTTP/2 for services that have `spec.ports[<port>].appProtocol` that is "kubernetes.io/h2c", "h2c", "http2", or "grpc".
...
Probing is a fallback mechanism for determining the protocol. It is recommended that it is avoided by setting the appropriate `appProtocol`. The probing takes place when the first connection is made to an intercepted service.