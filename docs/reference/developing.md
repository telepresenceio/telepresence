...
```console
$ ./build check TELEPRESENCE_REGISTRY=<Docker registry for tag and push> [PYTEST_ARGS=<pytest args>]
```
...
```console
$ ./build check TELEPRESENCE_REGISTRY=<Docker registry for tag and push> PYTEST_ARGS="-m 'vpn_tcp and swap_deployment'"
```
...
```console
$ ./build docker-push TELEPRESENCE_REGISTRY=<Docker registry for tag and push>
```
