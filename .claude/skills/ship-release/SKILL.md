...
The regression suite runs about 40 minutes; the node-agent job runs beside it. Use `ScheduleWakeup` with `delaySeconds` in the **1200-1800** range. Poll with the same `gh pr checks` query, looking at the `regression` and `node_agent_docker_runtime` rows.
